...
impl NativeTremolo {
    pub const ID: &'static str = "hardwave.native.tremolo";

    pub fn descriptor() -> PluginDescriptor {
        PluginDescriptor {
            id: Self::ID.into(),
            name: "Hardwave Tremolo".into(),
...
impl HostedPlugin for NativeTremolo {
    ...
    fn activate(&mut self, sr: f64, _max: u32) -> Result<(), String> {
        ...
        self.phase_l = 0.0;
        self.phase_r = self.stereo_phase;
        self.active = true;
        Ok(())
    }
    ...
    fn process(
        ...
        for i in 0..num_samples {
            // Map LFO -1..=1 to amp 1-depth..=1 (so depth=1 fully ducks)
            let lfo_l = lfo_value(self.shape, self.phase_l);
            let lfo_r = lfo_value(self.shape, self.phase_r);
            let amp_l = 1.0 - self.depth + self.depth * (0.5 + 0.5 * lfo_l);
            let amp_r = 1.0 - self.depth + self.depth * (0.5 + 0.5 * lfo_r);
            outputs[0][i] = inputs[0].get(i).copied().unwrap_or(0.0) * amp_l;
            outputs[1][i] = inputs[1].get(i).copied().unwrap_or(0.0) * amp_r;
            self.phase_l = (self.phase_l + dphase) % 1.0;
            self.phase_r = (self.phase_r + dphase) % 1.0;
        }
    }
    ...
    fn get_parameter_info(&self, index: u32) -> Option<ParameterInfo> {
        let (name, default, unit) = match index {
            PARAM_RATE => ("Rate", (5.0_f64 / 20.0).clamp(0.0, 1.0), "Hz"),
            PARAM_DEPTH => ("Depth", 0.5, "%"),
            PARAM_SHAPE => ("Shape", shape_to_norm(Shape::Sine), ""),
            PARAM_STEREO => ("Stereo", 0.0, ""),
            _ => return None,
        };
        Some(ParameterInfo {
            id: index,
    ...
    fn get_parameter_value(&self, id: u32) -> f64 {
        match id {
            ...
            PARAM_DEPTH => self.depth.clamp(0.0, 1.0) as f64,
            PARAM_SHAPE => shape_to_norm(self.shape),
            PARAM_STEREO => self.stereo_phase.clamp(0.0, 1.0) as f64,
            _ => 0.0,
        }
    }
