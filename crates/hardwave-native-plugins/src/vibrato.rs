//! Native vibrato — pitch wobble via 100% wet ModulatedDelay with NO
//! feedback and shorter base than chorus. Distinct from NativeChorus
//! (which mixes dry+wet for thickening) and NativeFlanger (which adds
...
impl NativeVibrato {
    pub const ID: &'static str = "hardwave.native.vibrato";

    pub fn descriptor() -> PluginDescriptor {
        PluginDescriptor {
            id: Self::ID.into(),
            name: "Hardwave Vibrato".into(),
    ...
    pub fn new() -> Self {
        ...
        let base = 0.005 * sr; // 5 ms — shorter than chorus (20 ms)
        delay_l.set_base_delay(base);
        delay_r.set_base_delay(base);
        delay_l.set_feedback(0.0);
...
impl Default for NativeVibrato {
    fn default() -> Self { Self::new() }
}

impl HostedPlugin for NativeVibrato {
    ...
    fn process(
        ...
        // 100% wet — vibrato is pure pitch modulation.
        for i in 0..num_samples {
            let in_l = inputs[0].get(i).copied().unwrap_or(0.0);
            let in_r = inputs[1].get(i).copied().unwrap_or(0.0);
            outputs[0][i] = self.delay_l.process(in_l);
            outputs[1][i] = self.delay_r.process(in_r);
        }
    }
    ...
    fn get_parameter_info(&self, index: u32) -> Option<ParameterInfo> {
        let (name, default, unit) = match index {
            PARAM_RATE => ("Rate", (5.0_f64 / 12.0).clamp(0.0, 1.0), "Hz"),
            PARAM_DEPTH => ("Depth", 1.5 / 5.0, "ms"),
            _ => return None,
        };
        Some(ParameterInfo {
    ...
    fn get_parameter_value(&self, id: u32) -> f64 {
        match id {
            PARAM_RATE => (self.rate_hz / 12.0).clamp(0.0, 1.0) as f64,
    ...
    fn get_state(&self) -> Vec<u8> {
        format!("{{\"rate\":{},\"depth\":{}}}", self.rate_hz, self.depth_ms).into_bytes()
    }
    ...
    fn set_state(&mut self, state: &[u8]) -> Result<(), String> {
        ...
        if let Some(v) = read("rate") { self.rate_hz = v.clamp(0.0, 12.0); }
