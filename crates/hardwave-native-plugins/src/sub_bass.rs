...
impl NativeSubBass {
    ...
    fn ensure_coefs(&mut self) {
        ...
        for f in [&mut self.lp_l, &mut self.lp_r] {
            f.set(BiquadKind::LowPass, self.sample_rate, self.crossover_hz, 0.707, 0.0);
        }
        for f in [&mut self.hp_l, &mut self.hp_r] {
            f.set(BiquadKind::HighPass, self.sample_rate, self.crossover_hz, 0.707, 0.0);
        }
...
impl HostedPlugin for NativeSubBass {
    ...
    fn get_parameter_info(&self, index: u32) -> Option<ParameterInfo> {
        let (name, default, unit) = match index {
            PARAM_CROSSOVER => (
                "Crossover",
                ((80.0_f64.log10() - 30.0_f64.log10()) / (300.0_f64.log10() - 30.0_f64.log10())).clamp(0.0, 1.0),
                "Hz",
            ),
    ...
    fn get_state(&self) -> Vec<u8> {
        format!(
            ...
            self.crossover_hz, self.boost_db, self.drive_l.amount(), self.mix
        ).into_bytes()
    }
    ...
    fn set_state(&mut self, state: &[u8]) -> Result<(), String> {
        ...
        if let Some(v) = read("cross") { self.crossover_hz = v.clamp(30.0, 300.0); self.needs_recoef = true; }
        if let Some(v) = read("boost") { self.boost_db = v.clamp(-12.0, 12.0); }
