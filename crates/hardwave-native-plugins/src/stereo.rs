...
impl HostedPlugin for NativeStereo {
    ...
    fn get_parameter_value(&self, id: u32) -> f64 {
        match id {
            ...
            PARAM_BASS_MONO => if self.bass_mono_on { 1.0 } else { 0.0 },
    ...
    fn set_parameter_value(&mut self, id: u32, value: f64) {
        ...
        match id {
            ...
            PARAM_CROSSOVER => {
                ...
                self.bass_mono.set_crossover(self.sample_rate, self.crossover_hz);
            }
            _ => {}
        }
    }
    ...
    fn get_state(&self) -> Vec<u8> {
        format!(
            ...
            self.width, self.balance, if self.bass_mono_on { 1 } else { 0 }, self.crossover_hz
        ).into_bytes()
    }
    ...
    fn set_state(&mut self, state: &[u8]) -> Result<(), String> {
        ...
        if let Some(v) = read("width") { self.width = v.clamp(0.0, 2.0); }
        if let Some(v) = read("balance") { self.balance = v.clamp(-1.0, 1.0); }
        if let Some(v) = read("bm") { self.bass_mono_on = v >= 0.5; }
