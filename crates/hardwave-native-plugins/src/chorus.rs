//! Native chorus — stereo modulated-delay pair with phase-offset LFOs.
...
const MAX_DELAY_SAMPLES: usize = 4096; // ~85 ms at 48 kHz
...
impl NativeChorus {
    pub const ID: &'static str = "hardwave.native.chorus";

    pub fn descriptor() -> PluginDescriptor {
        PluginDescriptor {
            id: Self::ID.into(),
            name: "Hardwave Chorus".into(),
    ...
    pub fn new() -> Self {
        ...
        let base = 0.020 * sr; // 20 ms base
        delay_l.set_base_delay(base);
        delay_r.set_base_delay(base);
        delay_l.set_lfo_rate(0.6);
        delay_r.set_lfo_rate(0.6);
        delay_l.set_lfo_depth(0.005 * sr);
        delay_r.set_lfo_depth(0.005 * sr);
        delay_l.set_lfo_phase_offset(0.0);
        delay_r.set_lfo_phase_offset(0.5); // 180° offset → stereo width
        delay_l.set_feedback(0.2);
        delay_r.set_feedback(0.2);
        Self {
    ...
    fn refresh(&mut self) {
        ...
        self.delay_l.set_lfo_depth(self.depth_samples);
        self.delay_r.set_lfo_depth(self.depth_samples);
        self.delay_l.set_lfo_phase_offset(0.0);
        // width 0..=1 → R-channel LFO offset 0..=0.5 cycles
        self.delay_r.set_lfo_phase_offset(self.width * 0.5);
        self.delay_l.set_feedback(self.feedback);
        self.delay_r.set_feedback(self.feedback);
    }
}
...
impl HostedPlugin for NativeChorus {
    ...
    fn activate(&mut self, sr: f64, _max: u32) -> Result<(), String> {
        ...
        let base = 0.020 * self.sample_rate;
        self.delay_l.set_base_delay(base);
        self.delay_r.set_base_delay(base);
        self.refresh();
        self.active = true;
        Ok(())
    }
    ...
    fn get_parameter_info(&self, index: u32) -> Option<ParameterInfo> {
        let (name, default, unit) = match index {
            PARAM_RATE => ("Rate", (0.6_f64 / 8.0).clamp(0.0, 1.0), "Hz"),
            PARAM_DEPTH => ("Depth", 0.5, "ms"),
            PARAM_FEEDBACK => ("Feedback", 0.2, "%"),
            PARAM_MIX => ("Mix", 0.4, "%"),
