...
fn thresh_from_norm(v: f64) -> f32 {
    (-60.0 + v.clamp(0.0, 1.0) * 60.0) as f32
}
fn thresh_to_norm(db: f32) -> f64 {
...
impl HostedPlugin for NativeMultiband {
    ...
    fn set_parameter_value(&mut self, id: u32, value: f64) {
        ...
        match id {
            ...
            PARAM_LOW_THRESH => {
                self.bands[0].threshold_db = thresh_from_norm(value);
                update_band = Some(0);
            }
            PARAM_LOW_RATIO => {
                self.bands[0].ratio = ratio_from_norm(value);
                update_band = Some(0);
            }
            PARAM_MID_THRESH => {
                self.bands[1].threshold_db = thresh_from_norm(value);
                update_band = Some(1);
            }
            PARAM_MID_RATIO => {
                self.bands[1].ratio = ratio_from_norm(value);
                update_band = Some(1);
            }
            PARAM_HI_THRESH => {
                self.bands[2].threshold_db = thresh_from_norm(value);
                update_band = Some(2);
            }
            PARAM_HI_RATIO => {
                self.bands[2].ratio = ratio_from_norm(value);
                update_band = Some(2);
            }
            PARAM_OUT_GAIN => {
    ...
    fn set_state(&mut self, state: &[u8]) -> Result<(), String> {
        ...
        if let Some(v) = read("mh") {
            self.mid_hi_hz = v;
        }
        if let Some(v) = read("lt") {
            self.bands[0].threshold_db = v;
        }
        if let Some(v) = read("lr") {
            self.bands[0].ratio = v;
        }
        if let Some(v) = read("mt") {
            self.bands[1].threshold_db = v;
        }
        if let Some(v) = read("mr") {
            self.bands[1].ratio = v;
        }
        if let Some(v) = read("ht") {
            self.bands[2].threshold_db = v;
        }
        if let Some(v) = read("hr") {
            self.bands[2].ratio = v;
        }
        if let Some(v) = read("og") {
